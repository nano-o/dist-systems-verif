...
lemma safe_mono:
  -- {* @{term safe_at} is monotonic. TODO: needed? *}
...
abbreviation safe where "safe s \<equiv> \<forall> i . ballot_array.safe (ballot s) (vote s i) quorums"
...
proof (auto simp add:ballot_array.safe_def split add:option.splits)
  ...
  next
    ...
    hence "proved_safe_at s i q (ballot s a) v" and "q \<in> quorums" and "\<forall> a2 \<in> q . ballot s a \<le> ballot s a2" 
      by (simp_all add:ballot_array.proved_safe_at_def split add:nat.splits)
    hence "safe_at s i v b" using quorums_axioms prems(1) prems(3) `ballot s a = b`
      ballot_array_props.proved_safe_at_imp_safe_at[of quorums "ballot s a" "vote s i" "ballot s" q v]
...
apply(rule invariantI)
  ...
  apply (meson agreement_def ballot_array_props.intro ballot_array_props.safe_imp_agreement quorums_axioms)
done
