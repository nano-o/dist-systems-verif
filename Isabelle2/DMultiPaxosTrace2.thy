...
| "ballot_2 a (e#es) = (let b = (ballot_2 a es) in
    case e of Phase1b a2 _ b2 _ \<Rightarrow> (if a = a2 then b2 else b) | _ \<Rightarrow> b)"
...
| "paxos ((Phase2a i b c) # es) = (safe_value i b (Some c) es \<and> paxos es)"
| "paxos ((VoteCmd a i b c) # es) = (suggestion i b es = Some c \<and> paxos es)"

lemma "\<lbrakk>paxos es; ballot_2 a es = b\<rbrakk> \<Longrightarrow> (started b es \<or> b = 0)" oops
...
lemma assumes "paxos es" and "a \<noteq> a2"
  shows "last_vote_2 a i es = None \<or> last_vote_2 a2 i es = None" quickcheck
...
fun propCmds :: "event list \<Rightarrow> cmd option set" where
  "propCmds [] = {}" |
  "propCmds (e#s) = (let pCmds = propCmds s in
    case e of Propose c \<Rightarrow> if Some c \<notin> pCmds then insert (Some c) pCmds else pCmds |  _ \<Rightarrow> pCmds)"

lemma "\<forall> S es . propCmds es = S" quickcheck oops
