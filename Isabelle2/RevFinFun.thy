...
(* Ian Added reverse linorder *)
datatype 'a new = New (old : 'a)
instantiation new :: (linorder) linorder
begin
definition "less_eq_new x y \<longleftrightarrow> old x \<ge> old y"
definition "less_new x y \<longleftrightarrow> old x > old y"
instance by (standard, case_tac [!] x) (auto simp: less_eq_new_def less_new_def)
end
...
where
  "finfun_to_list f = map old (THE xs. set xs = {x. finfun_dom f $ (old x)} \<and> sorted xs \<and> distinct xs)"

lemma set_finfun_to_list_aux: "set (THE xs. set xs = {x. finfun_dom f $ (old x)} \<and> sorted xs \<and> distinct xs) 
  = {x. finfun_dom f $ (old x)}" (is ?thesis1)
  and sorted_finfun_to_list: "sorted ((THE xs. set xs = {x. finfun_dom f $ (old x)} \<and> sorted xs \<and> distinct xs))" (is ?thesis2)
  and distinct_finfun_to_list: "distinct ((THE xs. set xs = {x. finfun_dom f $ (old x)} \<and> sorted xs \<and> distinct xs))" (is ?thesis3)
proof (atomize (full))
  have l:"finite {x. finfun_dom f $ (old x)}"
  proof -
    have "{New x | x . finfun_dom f $ x} = {x . finfun_dom f $ (old x)}" 
      by auto (metis new.exhaust_sel)
    moreover have  "finite {New x | x . finfun_dom f $ x}"
    proof -
      have "finite {x | x . finfun_dom f $ x}"
       using finite_finfun_dom by auto
      thus  "finite {New x | x . finfun_dom f $ x}" by auto
    qed
    ultimately show "finite {x. finfun_dom f $ (old x)}" by metis
  qed
  show "?thesis1 \<and> ?thesis2 \<and> ?thesis3" using finite_sorted_distinct_unique l
  proof -
    from finite_sorted_distinct_unique[of "{x. finfun_dom f $ (old x)}"] l
    have "\<exists>!xs. set xs = {x. finfun_dom f $ (old x)} \<and> sorted xs \<and> distinct xs"
      by auto
    thus ?thesis by (rule theI')
  qed
qed 
...
proof (atomize (full))
  have l:"\<And> f l . set (map f l) = {f x | x. x \<in> set l}" by auto
  have test:"(THE xs. set xs = {New x |x. finfun_dom f $ x} \<and> distinct xs \<and> sorted xs) = 
    map New (THE xs. set xs = {x |x. finfun_dom f $ x} \<and> distinct xs \<and> sorted xs)" sorry
  have "{x. finfun_dom f $ (old x)} = {New x | x . finfun_dom f $ x}"
      by auto (metis new.exhaust_sel)
  hence "set (map old (THE xs. set xs = {x. finfun_dom f $ (old x)} \<and> sorted xs \<and> distinct xs)) = 
    {old x |x. x \<in> set (THE xs. set xs = {New x |x. finfun_dom f $ x} \<and> sorted xs \<and> distinct xs)}" 
    using set_finfun_to_list_aux l[of old "THE xs. set xs = {x. finfun_dom f $ (old x)} \<and> sorted xs \<and> distinct xs"] by auto
  
  hence ?thesis1 using test unfolding finfun_to_list_def apply simp try0 oops
...
lemma finfun_to_list_update:
  ...
  (if b = finfun_default f then List.remove1 a (finfun_to_list f) else List.insort_insert_key New a (finfun_to_list f))"
proof(subst finfun_to_list_def, rule the_equality)
  ...
  show "xs = (if b = finfun_default f then remove1 a (finfun_to_list f) else insort_insert_key New a (finfun_to_list f))"
  ...
  next
    ...
    proof(cases "finfun_dom f $ a")
      ...
      qed
      thus ?thesis using False True eq by(simp add: insort_insert_key_triv)
    next
      case False
      ...
      thus ?thesis using False eq \<open>b \<noteq> finfun_default f\<close> 
        by (simp add: insort_insert_insort_key insort_key_remove1)
