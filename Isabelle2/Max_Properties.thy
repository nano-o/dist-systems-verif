...
lemma max_by_key_in:
  assumes "finite S" and "S \<noteq> {}" and "max_by_key_2 S f \<noteq> {}" and "m = max_by_key S f"
  shows "m \<in> max_by_key_2 S f" using assms
proof-
  {
    assume 0:"m \<notin> max_by_key_2 S f"
    have 1:"m \<in> S" using assms(1,2,4) by (meson all_not_in_conv max_by_key_in_and_ge(2))
    have 2:"f m = Max {f x | x . x \<in> S}" using 0 1 assms(1,4) 
      by (metis (no_types, lifting) in_and_ge_is_max_by_key_2 max_by_key_in_and_ge(1))
    hence "False" using 0 1 assms(1,4)  max_by_key_2_def
      by (metis (no_types, lifting) in_and_ge_is_max_by_key_2 max_by_key_in_and_ge(1))
  }
