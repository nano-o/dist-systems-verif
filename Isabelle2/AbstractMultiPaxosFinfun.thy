theory AbstractMultiPaxosFinfun
...
record ('v,'a) amp_state =
  propCmd :: "'v fset"
  ballot :: "'a \<Rightarrow>f nat option"
  vote :: "nat \<Rightarrow>f 'a \<Rightarrow>f nat \<Rightarrow>f 'v option"
...
definition amp_start where
  "amp_start \<equiv> {\<lparr>propCmd = {||}, ballot = (K$ None), vote = (K$ K$ K$ None) \<rparr>}"
...
apply (cases a)
    ...
    apply (smt finfun_upd_apply order.strict_trans) 
  (* vote *)
  apply (auto simp add:inv_proofs_defs ballot_array.safe_at_def ballot_array.choosable_def split add:option.split_asm)
  apply (smt antisym_conv2 dual_order.strict_implies_order finfun_apply_aux.finfun_rec_upd finfun_apply_def)
done
    ...
    proof (auto simp add:ballot_array.safe_def Let_def split add:option.split)
      ...
      proof (cases "i = j \<and> a\<^sub>2 = a")
        ...
        have "vote s $ j $ a\<^sub>2 $ b = vote t $ j $ a\<^sub>2 $ b" using False prems(2,5) vote_one_inst_only
          by (auto simp add:is_trans_def the_ioa_def amp_ioa_def amp_trans_def 
              split add:option.split_asm)
      ...
      next
        ...
        proof -
          have "vote s $ j $ a\<^sub>2 $ b = vote t $ j $ a\<^sub>2 $ b" using 5 that
            by auto 
          with l show ?thesis by auto
        qed
        moreover have ?thesis if "b = b\<^sub>2"
        proof -
          have "v = v\<^sub>2" using that True 2 5
            by (smt amp_state.select_convs(3) amp_state.surjective amp_state.update_convs(3) finfun_upd_apply option.inject) 
