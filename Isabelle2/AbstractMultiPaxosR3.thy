...
record ('v,'a,'l) ampr2_state =
  ...
  suggestion :: "inst \<Rightarrow> bal \<Rightarrow>f 'v option"
...
definition join_ballot where
  "join_ballot a b s s' \<equiv> 
    ...
    obs = (onebs s $ a $ b);
    onebs' = if (obs = None) then (fold (\<lambda> i ob. ob(i $:= (distributed_safe_at.acc_max (\<lambda> a b. vote s $ a $ i $ b) a b))) insts (K$ None))
      else (fold (\<lambda> i ob. ob(i $:= (distributed_safe_at.acc_max (\<lambda> a b. vote s $ a $ i $ b) a b))) insts (the obs))
    in
      b > (ballot s $ a) 
