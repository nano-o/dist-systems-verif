...
def finfun_rec[C, B, A](a: C => B, b: A => C => B => B, c: finfun[A, C]): B =
  sys.error("FinFun.finfun_rec")
...
def divide_nat(m: Nat.nat, n: Nat.nat): Nat.nat =
  Nat.Nata(Nat.divide_integer(Nat.integer_of_nat(m), Nat.integer_of_nat(n)))
...
import /*implicits*/ LinorderOption.ord_option, Nat.order_nat,
  FinFun.equal_finfun, Product_Type.equal_prod, Set.equal_list,
  Cardinality.card_UNIV_nat, Nat.linorder_nat, Optiona.equal_option,
  Nat.equal_nat
...
def accs(n: Nat.nat): FSet.fset[Nat.nat] =
  (if (Nat.equal_nata(n, Nat.zero_nat)) FSet.bot_fset[Nat.nat]
                                 ...
                                 FSet.finsert[Nat.nat](Nat.Suc(Nat.minus_nat(n,
                                      Nat.one_nat)),
...
def send_all[A, B, C](s: acc_state_ext[A, B], m: msg[C]): FSet.fset[packet[C]] =
  FSet.fimage[Nat.nat,
               packet[C]](((a2: Nat.nat) => Packet[C](id[A, B](s), a2, m)),
                           acceptors[A, B](s))

def send_all_2[A, B, C](s: acc_state_ext[A, B], a: Nat.nat, m: msg[C]):
      FSet.fset[packet[C]]
  =
  FSet.fimage[Nat.nat,
               packet[C]](((a2: Nat.nat) => Packet[C](id[A, B](s), a2, m)),
...
def do_2a[A, B](s: acc_state_ext[A, B], v: cmd[A]):
  ...
  =
  {
    ...
    val msg: msg[A] =
      Phase2a[A](inst, Optiona.the[Nat.nat](ballot[A, B](s)), v, a)
    val new_state: acc_state_ext[A, B] =
     ...
     FinFun.finfun[Nat.nat, List[Nat.nat]]])
      ...
      FinFun.finfun_const[List[Nat.nat], Nat.nat](List(a)))),
                          pending_update[A,
  B](((_: FinFun.finfun[Nat.nat, Option[cmd[A]]]) =>
      ...
      next_inst_update[A, B](((_: Nat.nat) => Nat.plus_nat(inst, Nat.one_nat)),
                              s)));
    (new_state, send_all_2[A, B, A](s, a, msg))
  }
...
def test_update[A, B](s: acc_state_ext[A, B], i: Nat.nat): acc_state_ext[A, B] =
  twobs_update[A, B](((_: FinFun.finfun[Nat.nat,
 FinFun.finfun[Nat.nat, List[Nat.nat]]])
                        =>
                       FinFun.finfun_update_code[Nat.nat,
  ...
  FinFun.finfun_update[Nat.nat,
                    ...
                    Nat.nat](Nil),
Nat.zero_nat, List(Nat.one_nat, Nat.nat_of_integer(BigInt(2)))))),
                      s)

def test_state(i: Nat.nat): acc_state_ext[Nat.nat, Unit] =
  {
    val s: acc_state_ext[Nat.nat, Unit] =
      init_acc_state[Nat.nat](Nat.nat_of_integer(BigInt(3)), Nat.one_nat)
    val sa: acc_state_ext[Nat.nat, Unit] = test_update[Nat.nat, Unit](s, i);
    sa
  }

def ebench[A](s: A): acc_state_ext[Nat.nat, Unit] =
  update_twobs[Nat.nat,
                Unit](test_state(Nat.nat_of_integer(BigInt(77))),
                       Nat.nat_of_integer(BigInt(97)), Nat.zero_nat,
                       List(Nat.one_nat, Nat.nat_of_integer(BigInt(2)),
                             Nat.nat_of_integer(BigInt(3))))
...
def ebench2: acc_state_ext[Nat.nat, Unit] =
  update_twobs[Nat.nat,
                Unit](test_state(Nat.nat_of_integer(BigInt(77))),
                       Nat.nat_of_integer(BigInt(97)), Nat.zero_nat,
                       new_twobs[Nat.nat,
                                  Unit](test_state(Nat.nat_of_integer(BigInt(77))),
 Nat.nat_of_integer(BigInt(97)), Nat.zero_nat, Nat.nat_of_integer(BigInt(3))))
...
def is_decided[A, B](s: acc_state_ext[A, B], i: Nat.nat): Boolean =
  ! (Optiona.is_none[cmd[A]](FinFun.finfun_apply[Nat.nat,
...
def receive_1b[A : HOL.equal](last_vs:
 ...
 s2)
            ...
            val pckts: List[FSet.fset[packet[A]]] =
              Set.map[msg[A],
                       FSet.fset[packet[A]]](((aa: msg[A]) =>
       send_all[A, Unit, A](s, aa)),
      msgs);
            (s3, Set.fold[FSet.fset[packet[A]],
...
def receive_2a[A](i: Nat.nat, b: Nat.nat, v: cmd[A], l: Nat.nat,
  ...
  =
  {
    ...
    (if (Optiona.equal_optiona[Nat.nat](bal, Some[Nat.nat](b)))
      (vote_update[A, Unit](((_: FinFun.finfun[Nat.nat, Option[cmd[A]]]) =>
                              FinFun.finfun_update_code[Nat.nat,
                 Option[cmd[A]]](vote[A, Unit](s), i, Some[cmd[A]](v))),
                             s),
        send_all[A, Unit, A](s, Phase2b[A](i, b, a, v)))
      else (s, FSet.bot_fset[packet[A]]))
  }
...
def serialize_finfun[A, B](ff: FinFun.finfun[A, B]): List[(A, B)] =
  FinFun.finfun_rec[B, List[(A, B)],
                     A](((_: B) => Nil),
                         ((a: A) => (b: B) =>
                           ((c: List[(A, B)]) => (a, b) :: c)),
                         ff)

def last_decision[A, B](x0: acc_state_ext[A, B]): Option[Nat.nat] = x0 match {
  case acc_state_exta(id, acceptors, ballot, vote, last_ballot, onebs, twobs,
                       decided, next_inst, pending, leader, last_decision, more)
    => last_decision
}
...
def get_last_decision[A, B](s: acc_state_ext[A, B]): cmd[A] =
  Optiona.the[cmd[A]](FinFun.finfun_apply[Nat.nat,
   Option[cmd[A]]](decided[A, B](s),
                    Optiona.the[Nat.nat](last_decision[A, B](s))))
...
def leader_1_init_state[A](n: Nat.nat): acc_state_ext[A, Unit] =
  acc_state_exta[A, Unit](Nat.one_nat, accs(n), Some[Nat.nat](Nat.one_nat),
                           FinFun.finfun_const[Option[cmd[A]], Nat.nat](None),
                           FinFun.finfun_const[Option[Nat.nat], Nat.nat](None),
                           FinFun.finfun_const[FinFun.finfun[Nat.nat,
                      List[(Nat.nat, Option[(cmd[A], Nat.nat)])]],
...
def main(args: Array[String]): Unit =
  {
    ...
    val stop = 100000;
    val command: Nat.nat = Nat.nat_of_integer(BigInt(9000))
    val v: cmd[Nat.nat] = Cmd[Nat.nat](command)
    var s: acc_state_ext[Nat.nat, Unit] =
      leader_1_init_state[Nat.nat](Nat.nat_of_integer(BigInt(3)))

    var t = System.nanoTime();
    for(a <- (1 to stop)) {
  
      if (a % 1000 == 1) {
        println( (a-1) + "," + ((System.nanoTime()-t)/1000.0));
        t=System.nanoTime(); 
      }
       s=Product_Type.fst[acc_state_ext[Nat.nat, Unit],FSet.fset[packet[Nat.nat]]](propose[Nat.nat](command,s))            

       s=Product_Type.fst(receive_2b(Nat.nat_of_integer(BigInt(a)),
                                 Optiona.the(ballot(s)),
                                 Nat.nat_of_integer(BigInt(2)), v, s))
       s=Product_Type.fst(receive_2b(Nat.nat_of_integer(BigInt(a)),
                                 Optiona.the(ballot(s)),
                                 Nat.nat_of_integer(BigInt(3)), v, s));
