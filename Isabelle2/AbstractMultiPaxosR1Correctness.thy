...
definition inv1 where inv1_def[inv_proofs_defs]:
  ...
  "inv1 s \<equiv> \<forall> a i b . let v = vote s a i b in v \<noteq> None \<longrightarrow> v = suggestion s i b"
...
abbreviation conservative_array where
  "conservative_array s \<equiv>  \<forall> i . ballot_array.conservative_array (\<lambda>a. vote s a i)"
...
lemma trans_imp_prefix_order:
  ...
  shows "is_prefix (ballot s) (ballot t) (\<lambda>a. vote s a i) (\<lambda>a. vote t a i)" using assms
...
abbreviation safe_at where "safe_at s i \<equiv> ballot_array.safe_at quorums (ballot s) (\<lambda>a. vote s a i)"
...
definition inv3 where inv3_def[inv_proofs_defs]:
  "inv3 s \<equiv> \<forall> a b . case onebs s a b of None \<Rightarrow> True
    | Some maxs \<Rightarrow> 
      (\<forall> i . maxs i = distributed_safe_at.acc_max (\<lambda>a. vote s a i) a b)
      \<and> (ballot s a \<ge> b)"
...
apply ((induct_tac rule:trans_cases, simp); rm_trans_rel_assm)
    ...
    proof -
      { fix a2 b maxs
        ...
        from a1 prems(1) have 1:"\<And> j . maxs j = distributed_safe_at.acc_max (\<lambda>a. vote s a j) a2 b" 
        ...
        { fix j assume "j \<noteq> i \<or> a \<noteq> a2"
          hence "maxs j = distributed_safe_at.acc_max (\<lambda>a. vote t a j) a2 b" using 1 \<open>do_vote a i v s t\<close> by (auto simp add:distributed_safe_at.acc_max_def) }
        note 3 = this
        { fix j assume "j = i \<and> a = a2"
          hence "maxs j = distributed_safe_at.acc_max (\<lambda>a. vote t a j) a2 b" using 1 \<open>do_vote a i v s t\<close> a1 2 apply (simp add:distributed_safe_at.acc_max_def)
...
abbreviation safe where "safe s \<equiv> \<forall> i . ballot_array.safe quorums (ballot s) (\<lambda>a. vote s a i)"
...
lemma aqcuire_leadership_ok:
  ...
  shows "let safe_at = \<lambda> v . ballot_array.safe_at quorums (ballot t) (\<lambda>a. vote t a i) v (ballot t a) in
...
proof -
  ...
  let ?proved_safe_at = "distributed_safe_at.proved_safe_at quorums (ballot t) (\<lambda>a. vote t a i) q ?b"
  ...
  proof -  
    ...
    have "distributed_safe_at.max_pair q (\<lambda> a2 . the (onebs t a2 ?b) i) = 
      distributed_safe_at.max_pair q (\<lambda> a2 . distributed_safe_at.acc_max (\<lambda>a. vote t a i) a2 ?b)"
    proof -
      have "distributed_safe_at.max_pair q (\<lambda> a2 . the (onebs s a2 ?b) i) = 
        distributed_safe_at.max_pair q (\<lambda> a2 . distributed_safe_at.acc_max (\<lambda>a. vote s a i) a2 ?b)"
      proof -
        have "\<And> a2 . a2 \<in> q \<Longrightarrow> case onebs s a2 ?b of 
                  Some f \<Rightarrow> f i = distributed_safe_at.acc_max (\<lambda>a. vote s a i) a2 ?b 
  ...
  interpret p:dsa_properties "ballot t" "\<lambda>a. vote t a i" quorums by unfold_locales
    ...
    apply ((induct_tac rule:trans_cases, simp); rm_trans_rel_assm)
        ...
        next
          show "inv8 t"  proof (auto simp add:inv8_def)
            ...
            next
              case True thus ?thesis using aqcuire_leadership_ok \<open>inv3 s\<close> \<open>safe s\<close> \<open>\<forall>i. ballot_array.conservative_array (\<lambda>a. vote t a i)\<close> 
                \<open>acquire_leadership a q s t\<close> a2 by (metis case_optionE option.simps(3))
            qed
          qed
        next
          show "inv2 t" proof (auto simp add:inv2_def split add:option.splits)
            ...
            next
              ...
              show ?thesis using 1 True aqcuire_leadership_ok[OF \<open>acquire_leadership a q s t\<close> \<open>inv3 s\<close> \<open>safe s\<close> \<open>\<forall>i. ballot_array.conservative_array (\<lambda>a. vote t a i)\<close>, of i]  
      ...
      proof (rule conjI; (rule conjI)?) thm prems(12,14,9)
        show "inv8 t" using \<open>inv8 s\<close> \<open>do_vote a i v s t\<close> unfolding amp_ioa.do_vote_def inv8_def
          by (smt amp_state.ext_inject amp_state.surjective amp_state.update_convs(3) prems(1))
      next
        show "inv2 t" using prems(1) \<open>inv2 s\<close> \<open>do_vote a i v s t\<close> unfolding amp_ioa.do_vote_def inv2_def 
          by (smt amp_state.ext_inject amp_state.surjective amp_state.update_convs(3) option.case_eq_if) 
      next
        show "safe t" using prems(1) \<open>do_vote a i v s t\<close> \<open>inv2 s\<close> \<open>inv1 t\<close> unfolding amp_ioa.do_vote_def ballot_array.safe_def inv1_def inv2_def 
          by (smt amp_state.ext_inject amp_state.surjective amp_state.update_convs(3) option.case_eq_if ) 
      qed
    done
  qed
...
definition inv5 where
  inv5_def[inv_proofs_defs]:"inv5 s \<equiv> \<forall> a i b . ballot s a < b \<longrightarrow> vote s a i b = None"
...
apply (metis option.distinct(1)) by blast
...
unfolding  ballot_array.chosen_def ballot_array.chosen_at_def
proof -
  fix s :: "('v, 'a, 'l) amp_state" and a :: "('v, 'a, 'l) amp_action" and aa :: 'a and i :: nat and v :: 'v and ia :: nat and l :: 'l and x2 :: 'v
  assume a1: "\<forall>i l x2. learned s l i = Some x2 \<longrightarrow> (\<exists>b q. q \<in> quorums \<and> (\<forall>a\<in>q. vote s a i b = Some x2))"
  assume a2: "vote s aa i (ballot s aa) = None"
  assume a3: "learned s l ia = Some x2"
  { fix aaa :: "nat \<Rightarrow> 'a set \<Rightarrow> 'a"
    obtain AA :: "nat \<Rightarrow> 'l \<Rightarrow> 'v \<Rightarrow> 'a set" and nn :: "nat \<Rightarrow> 'l \<Rightarrow> 'v \<Rightarrow> nat" where
      ff1: "\<forall>n l v. learned s l n \<noteq> Some v \<or> AA n l v \<in> quorums \<and> (\<forall>a. a \<notin> AA n l v \<or> vote s a n (nn n l v) = Some v)"
      using a1 by (metis (no_types))
    have "aaa (nn ia l x2) (AA ia l x2) = aa \<longrightarrow> (\<exists>n. (if aaa n (AA ia l x2) = aa then (vote s aa)(i := vote s aa i(ballot s aa \<mapsto> v)) else vote s (aaa n (AA ia l x2))) ia n = Some x2) \<or> (\<exists>n. vote s (aaa n (AA ia l x2)) ia (nn ia l x2) \<noteq> Some x2)"
      using a2 by force
    moreover
    { assume "aaa (nn ia l x2) (AA ia l x2) \<noteq> aa"
      then have "(\<exists>n. vote s (aaa n (AA ia l x2)) ia (nn ia l x2) \<noteq> Some x2) \<or> (\<exists>n. (if aaa n (AA ia l x2) = aa then (vote s aa)(i := vote s aa i(ballot s aa \<mapsto> v)) else vote s (aaa n (AA ia l x2))) ia n = Some x2)"
        by force }
    ultimately have "\<exists>n A. A \<in> quorums \<and> (aaa n A \<notin> A \<or> (if aaa n A = aa then (vote s aa)(i := vote s aa i(ballot s aa \<mapsto> v)) else vote s (aaa n A)) ia n = Some x2)"
      using ff1 a3 by blast }
  then show "\<exists>n A. A \<in> quorums \<and> (\<forall>a\<in>A. (if a = aa then (vote s aa)(i := vote s aa i(ballot s aa \<mapsto> v)) else vote s a) ia n = Some x2)"
    by (metis (no_types))
qed
(*apply (metis (mono_tags) amp_state.select_convs(3) amp_state.surjective amp_state.update_convs(3) fun_upd_same)
...
proof -
  { fix s
    ...
    qed }
  thus ?thesis using inv4 inv8_inv2_safe unfolding IOA.invariant_def by blast
qed
