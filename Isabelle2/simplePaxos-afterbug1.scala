...
def do_2a[A, B](s: acc_state_ext[A, B], v: cmd[A]):
  ...
  =
  {
    ...
    val new_state: acc_state_ext[A, B] =
      pending_update[A, B](((_: FinFun.finfun[Nat.nat, Option[cmd[A]]]) =>
                             FinFun.finfun_update_code[Nat.nat,
                Option[cmd[A]]](pending[A, B](s), inst, Some[cmd[A]](v))),
                            next_inst_update[A,
      B](((_: Nat.nat) => Nat.plus_nat(inst, Nat.one_nat)), s));
    (new_state, send_all[A, B, A](s, msg))
  }
...
def test_state(i: Nat.nat): acc_state_ext[Nat.nat, Unit] =
  {
    ...
    val start= 1;
    val stop = Nat.integer_of_nat(i).intValue;
    val step = 1;

    for(a <- (start to stop by step)) {

      s = test_update(s,Nat.nat_of_integer(BigInt(a)))
...
def main(args: Array[String]): Unit =
  {
    ...
    for(a <- (start to stop by step)) {
      val ts = test_state(Nat.nat_of_integer(BigInt(a)));


      val t0 = System.nanoTime();
    //  val arraya: FinFun.finfun[Nat.nat, Nat.nat] =
      for(b <- (1 to repcount)) {
        update_twobs[Nat.nat,
                Unit](ts,
                       Nat.nat_of_integer(BigInt(a-3)), Nat.zero_nat,
                       List(Nat.one_nat, Nat.nat_of_integer(BigInt(2)),
                             Nat.nat_of_integer(BigInt(3))))
...
//      val arrayb: FinFun.finfun[Nat.nat, Nat.nat] =
//        mb2b(array, a, Nat.nat_of_integer(BigInt(10)));
//      val t3 =  System.nanoTime();
//      println( (t1-t0) + "," + (t3-t2) + "," + a2 );
      println( ((t1-t0)/repcount.toFloat) + ","  + a );
    }
  }
