...
datatype 'v inst_info = inst_decided "'v cmd" | inst_pending "'v cmd" bal | not_seen
...
| Phase2a inst (for_ballot:bal) (suggestion:"'v cmd") (leader: acc)
| Phase2b inst (ballot:bal) (acceptor: acc) (cmd: "'v cmd")
| Fwd (val: 'v)
  -- {* Forwards a proposal to another proposer (the leader) *}
| Stop
| StopAck acc "inst \<Rightarrow>f 'v inst_info"
...
record 'v acc_state =
  ...
  onebs :: "bal \<Rightarrow>f inst \<Rightarrow>f (acc \<times> ('v cmd \<times> bal) option) list"
    -- {* For an acceptor a and a ballot b,
      lists of 1b-message descriptions, indexed by ballot then instance. *}
  ...
  stopped :: "bool"
  stop_acks :: "acc \<Rightarrow>f inst \<Rightarrow>f 'v inst_info"
  aborted :: "'v cmd list option"
...
text {* output transitions could return an option *}
...
definition request_stop where
  "request_stop a s \<equiv> (s, send_all s a Stop)"

definition inst_info where 
  "inst_info s i \<equiv> case (decided s $ i) of Some c \<Rightarrow> inst_decided c | None \<Rightarrow> (
        inst_pending (the (pending s $ i)) (the (last_ballot s $ i)))"

definition insts_info_acc where "insts_info_acc s \<equiv> \<lambda> i f_rec . f_rec(i $:= inst_info s i)"

interpretation inst_info_commute: comp_fun_commute "insts_info_acc s" for s
using FinFun.finfun_comp_aux.upd_left_comm unfolding insts_info_acc_def
by fast

interpretation inst_info_idem:folding_idem "insts_info_acc s" "K$ not_seen" 
  for s unfolding insts_info_acc_def
apply (unfold_locales)
apply (simp add:comp_def fun_eq_iff)
apply auto
by (metis finfun_update_twist)

definition insts_info where
  "insts_info s is \<equiv> Finite_Set.fold (insts_info_acc s) (K$ not_seen) is"

lemma[code]: "insts_info s (set xs)
  = fold (insts_info_acc s) xs (K$ not_seen)"
proof (induct xs)
  case Nil thus ?case apply(simp only:insts_info_def)
    by (metis fold_empty fold_simps(1) list.set(1))
next
  case (Cons x xs) thus ?case
    apply (simp only:insts_info_def)
    by (metis (mono_tags, lifting) List.finite_set fold_commute_apply fold_simps(2)
      folding.comp_fun_commute folding_idem.insert_idem inst_info_idem.eq_fold
      inst_info_idem.folding_axioms inst_info_idem.folding_idem_axioms list.simps(15))
qed

definition stop where
  "stop s \<equiv> (s\<lparr>stopped := True\<rparr>,
    send_all s (id s) (StopAck (id s) (insts_info s {1..<(next_inst s)})))"

definition rcv_stop_ack where
  "rcv_stop_ack f a s \<equiv> (s\<lparr>stop_acks := (stop_acks s)(a $:= f)\<rparr>, {||})"

definition abort where 
  "abort a s \<equiv> if (2 * fcard (finfun_fset_dom (stop_acks s)) > (nr s))
    then let
        acks = stop_acks s;
...
| "process_msg Stop s = stop s"
| "process_msg (StopAck a f) s = rcv_stop_ack f a s"
