...
imports 
  MultiPaxos3 "../../IO-Automata/IOA_Automation" "../../IO-Automata/Simulations"
begin
...
record 'v acc_state =
  ...
  vote_hist :: "inst \<Rightarrow>f bal \<Rightarrow>f 'v cmd option"
...
definition receive_2a :: "inst \<Rightarrow> bal \<Rightarrow> 'v cmd \<Rightarrow> acc \<Rightarrow> 'v acc_state \<Rightarrow> ('v acc_state \<times> 'v packet fset)" where
  "receive_2a i b v l s \<equiv>
    (let a = id s; bal = (ballot s) in
      (if (bal = Some b)
        then (s\<lparr>vote := (vote s)(i $:= Some v), 
          vote_hist := (vote_hist s)(i $:= (vote_hist s $ i)(b $:= Some v))\<rparr>, send_all s (Phase2b i b a v))
        else (s, {||})))"
...
record 'v mp_state = 
  node_states :: "acc \<Rightarrow>f 'v acc_state"
...
export_code learn send_1a propose process_msg in Scala file "multiPaxos.scala"
