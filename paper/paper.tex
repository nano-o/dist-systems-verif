...
histories: $h_1 \leq h_2$ when there exists a sequence $cs$ such that $h_2 =
h_1 \star cs$. 
The maximum over a set of
histories $H$ is denoted by $Max\left( H \right)$ and $Max\left( \aset{}
\right)$ is the empty history.
Histories have the property that every finite set $H$ of 
histories has a \emph{greatest lower bound}, noted $GLB\left(H\right)$.
Finally, we say that a set of histories $H$ is compatible when all the histories in $H$ have a common upper bound $h$ such
that $h$ can be built using only commands found in the histories in $H$.
