...
\subsection{The Broadcast Algorithm}

In our first algorithm, Broadcast, learners each maintain a variable $hist\left[ l \right]$ which contain the currently accepted history of learner $l$.
...
More precisely, the Broadcast algorithm is described by the transition relation obtained as the disjunction of the
...
To overcome this deadlock, we will make some modifications to the algorithm that will allow us to start a fresh instance $B_2$, called a round, of the Broadcast algorithm.
To allow switching to a fresh new instance of the Broadcast algorithm, we introduce the new role of switcher, that any server can play additionally from being a proposer, acceptor, or learner.
...
We write $B_1$ for the first round of the Broadcast algorithm.
...
We modify the Broadcast algorithm by introducing new transitions and modifying the $Accept\left( a \right)$ transition as follows.
...
\item The $Init\left( s \right)$ transition, for a switcher $s$, is enabled when $inits\left[ s \right]=\quo{none}$, and its effect is to pick a history $h$, set $inits\left[ s \right]$ to $h$, and to broadcast the message $\aseq{\quo{init}, h}$ to the acceptors.  The history $inits\left[ s \right]$ is then called an init history.
...
The two rounds $B_1$ and $B_2$ are composed as follows:
\begin{itemize}
...
The Broadcast algorithm can process a proposal in two message delays when there is no contention on non-commuting commands. However, if there is
...
designating a leader server that will totally order the proposals. However, to make our Broadcast algorithm work like Fast Paxos, we would have to modify it substantially. If we had spent effort testing Broadcast implementations and proving it correct, this effort would be wasted and we would have to redo all the tests and proofs from scratch.

Instead, we will now show how to obtain an algorithm with performance characteristics close to those of Fast Paxos, employing a leader to deal with contention, without modifying at all our existing Broadcast algorithm.
To do so, we define a new kind of rounds, the Simple-Leader rounds, which uses the same inter-round interface as the Broadcast algorithm. 
The inter-round interface will allow use to combine Simple-Leader rounds and Broadcast rounds in any order, obtaining an algorithm which can adapt its strategy to its environment, selecting the Broadcast algorithm under low contention and switching to the more expensive Simple-Leader algorithm when there is contention.
...
As in the Broadcast algorithm, acceptors broadcast their accepted histories to the learners, which learn a new history when it is a lower bound of 
...
To recover from this deadlock we pass the baton to a new round, as in the Broadcast algorithm, with a similar same mechanism: a switcher that times-out waiting for new messages from the acceptors broadcast a stop message, gathers the accepted history from at least one of the acceptors, and aborts. 
...
Thanks to the inter-round interface, Simple-Leader rounds and Broadcast rounds can be composed in any order.
To compose a Simple-Leader round $SL_1$ with an Broadcast round $ABA_2$, we compose their specifications as before, adding the constraint that the set $aborts_1$ be equal to the set $inits_2$ at all times.
